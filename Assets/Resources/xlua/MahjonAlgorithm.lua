...
function MahjonAlgorithm.IsPeng(handcard, gocard)
    local mid = MahjonAlgorithm.TraverseSpecifiedNumber(handcard,2)
    if (#mid >= 2) then
...
function MahjonAlgorithm.IsGang(handcard, gocard, number)
    local mid = MahjonAlgorithm.TraverseSpecifiedNumber(handcard, number)
    if (#mid >= number) then
        for k,card in pairs(mid) do
            if (gocard:getSize() == card:getSize() and gocard:getCardMark() == card:getCardMark()) then
                return true
...
function MahjonAlgorithm.IsHu(tbHandcard)
    ...
    return MahjonAlgorithm.TryHu(count, #tbHandcard)
end
    ...
    if (cardNumber % 3 == 2) then
        ...
        for i = 1, 27 do
            if (tbCount[i] >= 2) then
                ...
                if (MahjonAlgorithm.TryHu(tbCount, cardNumber - 2)) then                
                    return true
                else
                    tbCount[i] = tbCount[i] + 2
                end
    ...
    elseif (cardNumber % 3 == 0) then
        ...
        for i = 1, 27 do
            if (tbCount[i] >= 3) then
                ...
                if (MahjonAlgorithm.TryHu(tbCount, cardNumber - 3)) then
                    return true
                else
                    tbCount[i] = tbCount[i] + 3
                end
